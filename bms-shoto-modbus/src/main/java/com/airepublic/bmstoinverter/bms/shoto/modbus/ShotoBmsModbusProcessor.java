...
public class ShotoBmsModbusProcessor extends BMS {
    ...
    protected void readBatteryVoltage(final ByteBuffer frame) {
        final int bmsId = frame.getInt(); // unitId
    ...
    protected void readMaxDischargeVoltage(final ByteBuffer frame) {
        ...
        pack.minPackVoltageLimit = frame.getInt() / 100;
    }
    ...
    protected void readBatteryStatus(final ByteBuffer frame) {
        ...
        pack.packSOC = frame.getInt() / 10;
        pack.packSOH = frame.getInt() / 10;
        pack.numOfTempSensors = frame.getInt();

        short alarms = (short) frame.getInt();
LOG.info(String.format("Alarm1: %x\n", alarms));
        ...
        alarms = (short) frame.getInt();
LOG.info(String.format("Alarm2: %x\n", alarms));
        ...
        alarms = (short) frame.getInt();
LOG.info(String.format("Alarm3: %x\n", alarms));
        highByte = (byte) (alarms >> 8);
        ...
        alarms = (short) frame.getInt();
LOG.info(String.format("Alarm4: %x\n", alarms));
        alarms = (short) frame.getInt();
LOG.info(String.format("Alarm5: %x\n", alarms));
        highByte = (byte) (alarms >> 8);
...
LOG.info("moduleRatedCapacityAh:  " + pack.moduleRatedCapacityAh);
    }
